...
int main() {
  ...
  const std::string listen_addr =
      balanz::getenv_or("GRPC_ADDR", "0.0.0.0:50052");
  const std::string publisher_addr =
      balanz::getenv_or("PUBLISHER_ADDR",
                        balanz::getenv_or("NATS_GRPC_ADDR", "localhost:50051"));

  auto channel = grpc::CreateChannel(publisher_addr,
                                     grpc::InsecureChannelCredentials());
  auto nats_stub = nats::v1::NatsPublisherService::NewStub(channel);

  balanz::IngestServiceImpl service(std::move(nats_stub));
  ...
  spdlog::info("Ingest gRPC listening on {} (calls NATS publisher at {})",
