...
void Interactive() {
    ...
    // This channel models a connection to an endpoint (in this case, localhost at port 50055). 
    // We indicate that the channel isn't authenticated (use of InsecureChannelCredentials()).
    GreeterClient greeter(grpc::CreateChannel("localhost:50055", grpc::InsecureChannelCredentials()));
    while (true) {
        std::string user;
        ...
        std::cout << "rpc returned: " << std::endl;
